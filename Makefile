...
help:
	...
	@echo "build           -> build the binary in dist folder"

updateVersion:
	sed -i -r 's/(const char\* software_version = ")[^"]+(";$$)/\1'$(VERSION)'\2/' $(SRC_FILE)

build: updateVersion
	arduino --board esp8266:esp8266:generic:CpuFrequency=80,UploadSpeed=115200,FlashFreq=40,FlashSize=4M3M --pref build.path=$$PWD/dist --verify $(SRC_FILE)
